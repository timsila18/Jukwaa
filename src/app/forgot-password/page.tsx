...
export default function ForgotPasswordPage() {
  ...
  async function submit() {
    ...
    setStatus(`Reset code: ${payload.resetCode}. It expires in ${payload.expiresIn}.`);
  }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-10 text-slate-900">
      <section className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">Generate a one-time reset code. Use it immediately to create a new password.</p>
        <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
          ...
          <button className="h-11 w-full rounded-md bg-slate-950 px-4 text-sm font-bold text-white hover:bg-slate-900" type="submit">Generate Reset Code</button>
        </form>
