...
export default function ActivationPage() {
  ...
  async function activate() {
    ...
    setStatus(`Workspace activated for candidate ${payload.candidateId}. Mode: ${payload.activationMode ?? "Admin Approved"}.`);
  }
    ...
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-8 text-slate-900">
      <section className="w-full max-w-xl rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">Confirm payment when available, or approve a candidate workspace before payment when an administrator authorizes early access.</p>
        </div>
        <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); void activate(); }}>
          ...
          <label className="block text-sm font-semibold text-slate-700">Payment ID <span className="font-normal text-slate-500">(optional for admin approval)</span><input className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-sky-500" onChange={(event) => setPaymentId(event.target.value)} value={paymentId} /></label>
          <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm font-semibold leading-6 text-amber-900">
            If Payment ID is blank, this approves the workspace for use before payment. Keep a manual note and follow up on payment collection.
          </div>
          <button className="h-11 w-full rounded-md bg-slate-950 px-4 text-sm font-bold text-white hover:bg-slate-900" type="submit">Approve Workspace Access</button>
        </form>
