  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-8 text-slate-900">
      <section className="w-full max-w-xl rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-6">
          <div className="grid h-11 w-11 place-items-center rounded-lg bg-teal-50 text-teal-700"><ShieldCheck size={20} /></div>
        ...
        <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); void activate(); }}>
          <label className="block text-sm font-semibold text-slate-700">Application ID<input className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-teal-500" onChange={(event) => setApplicationId(event.target.value)} value={applicationId} /></label>
          <label className="block text-sm font-semibold text-slate-700">Payment ID<input className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-teal-500" onChange={(event) => setPaymentId(event.target.value)} value={paymentId} /></label>
          <button className="h-11 w-full rounded-md bg-teal-700 px-4 text-sm font-bold text-white hover:bg-teal-800" type="submit">Activate Workspace</button>
        </form>
