...
export default function ResetPasswordPage() {
  ...
  async function submit() {
    ...
    setStatus(payload.status ?? "Password updated. You can now login.");
  }
    ...
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-10 text-slate-900">
      <section className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
          ...
          <label className="block text-sm font-semibold text-slate-700">New password<input autoComplete="new-password" className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-sky-500" onChange={(event) => update("password", event.target.value)} type="password" value={form.password} /></label>
