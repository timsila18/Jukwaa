  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-50 px-4 py-8 text-slate-900">
      <section className="w-full max-w-2xl rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-6">
          <div className="grid h-11 w-11 place-items-center rounded-lg bg-teal-50 text-teal-700"><Smartphone size={20} /></div>
        ...
        <form className="mt-6 grid gap-4 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); void submitPayment(); }}>
          <label className="block text-sm font-semibold text-slate-700">Application ID<input className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-teal-500" onChange={(event) => update("applicationId", event.target.value)} value={form.applicationId} /></label>
          <label className="block text-sm font-semibold text-slate-700">Account reference<input className="mt-1 h-11 w-full rounded-md border border-slate-200 px-3 text-sm outline-none focus:border-teal-500" onChange={(event) => update("accountReference", event.target.value)} value={form.accountReference} /></label>
